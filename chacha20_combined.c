// Combined ChaCha20 implementation for SAW verification
// This file combines chacha20.c and chacha20block.c
...
int32_t CRYPT_CHACHA20_SetCount(CRYPT_CHACHA20_Ctx *ctx, const uint8_t *cnt, uint32_t cntLen)
{
...
// CHACHA20_Update from chacha20block.c (modified version)
    ...
    uint8_t *out, uint32_t len)
{
    ...
    while (tLen >= CHACHA20_STATEBYTES) {
        ...
        for (uint32_t ii = 0; ii < CHACHA20_STATEBYTES; ii++) {
            offOut[ii] = offIn[ii] ^ ctx->last.u[ii];
        }
...
// CRYPT_CHACHA20_Update from chacha20.c
int32_t CRYPT_CHACHA20_Update(CRYPT_CHACHA20_Ctx *ctx, const uint8_t *in, uint8_t *out, uint32_t len)
{
    ...
    if (tLen > 0) {
        ...
        if (t != 0) {
            // Simple DATA64_XOR replacement for 8-byte aligned processing
            for (i = 0; i < t; i += 8) {
                *((uint64_t*)(offOut + i)) = *((uint64_t*)(ctx->last.u + i)) ^ *((uint64_t*)(offIn + i));
            }
        }
        for (i = t; i < tLen; i++) {
